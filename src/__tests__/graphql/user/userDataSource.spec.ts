import {createMockContext, MockContext} from "../../../config/prisma/testClient";
...
describe("getMany user", () => {
    it("should return all users", async () => {
        const mockUsers: PrismaUser[] = [
            {id: "123", role: "USER", email: "test1@email.com", password: "password", updatedAt: new Date(Date.now()), createdAt: new Date(Date.now())},
            {id: "124", role: "USER", email: "test2@email.com", password: "password", updatedAt: new Date(Date.now()), createdAt: new Date(Date.now())},
            {id: "126", role: "USER", email: "test3@email.com", password: "password", updatedAt: new Date(Date.now()), createdAt: new Date(Date.now())},
        ]
...
describe("getOneByEmailAndPassword", () => {
    it("should return a user", async () => {
        ...
        const returnedUser = await userDateSource.getOneByEmailAndPassword("test1@gmail.com", "password")
        expect(returnedUser.id).toBe(mockUser.id)
    })
    ...
    it("should throw if password doesn't match", async () => {
        ...
        await expect(userDateSource.getOneByEmailAndPassword(email, password))
            .rejects
            .toEqual(new UserInputError("Incorrect Username / Password"))
    })
})
...
describe("createOne User", () => {
    it("should return a created user", async () => {
        ...
        const returnedUser = await userDateSource.createOne("test1@email.com", "password")
    ...
    it("should throw if user already exists", async () => {
        ...
        await expect(userDateSource.createOne("test@email.com", "password"))
            .rejects
            .toEqual(new UserInputError("User already exists"))
    })
})
...
describe("updateOne User", () => {
    it("should return a user", async () => {
        ...
        const returnedUser = await userDateSource.updateOne("123", "test1@email.com", "password")
